...
### Your code here
Trips %>%
  ggplot(aes(x = client, y = as.numeric(edate - sdate))) +
  geom_boxplot() +
  ylab("seconds") +
  ylim(0, 4500)
...
Its very lumpy, but it starts going up and goes up till 300 and start going down slowly to a fast pace.
...
My observations here..... 
Has a very consistent wave pattern but some are shorter than the others.
...
My observations here..... 
Was slowly going down to up until 16-ish to then go back down.
...
My observations here..... 
Looks like jello to me or a rock that was polished on all sides except for the top.
...
GlyphReadyData <-
  Trips %>%
  mutate( hour_of_the_day = lubridate::wday(sdate), 
          day_of_the_week = lubridate::hour(sdate)) %>%
  select(hour_of_the_day, day_of_the_week, client)

GlyphReadyData %>%
  ggplot(aes(x = hour_of_the_day)) +
  geom_density() +
  facet_grid( ~ client )
...
My comments here.....
For hour of the day the casual are shaped like a wide "u", while the registered are spiky. For day of the week the casual looks like a bell-shaped curve, while the registered looks like the spider-man shooting webs symbol.
...
PairDistancesJoined <-
  PairDistances %>%
  inner_join(Trips, by = c("sstation", "estation")) %>%
  select(sstation, estation, distance)
